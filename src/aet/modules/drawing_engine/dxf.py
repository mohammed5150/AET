...
from ezdxf.entities import DXFEntity, Insert, LWPolyline, MText, Polyline
...
class DxfInterpreter:
    ...
    def _block_facts(self, document: Drawing) -> dict[str, _BlockFacts]:
    ...
    def _convert(
        self, raw: DXFEntity, blocks: dict[str, _BlockFacts] | None = None
    ) -> DrawingEntity | None:
        ...
        if entity_type == "line":
            ...
            lwpolyline = cast("LWPolyline", raw)
            geometry = {
                "points": [_xy(p) for p in lwpolyline.get_points("xy")],
                "closed": bool(lwpolyline.closed),
            }
        elif entity_type == "polyline":
            polyline = cast("Polyline", raw)
            geometry = {
                "points": [_xy(v.dxf.location) for v in polyline.vertices],
                "closed": bool(polyline.is_closed),
            }
            ...
            attributes.update(self._block_attributes(cast("Insert", raw)))
            ...
            geometry = {"position": _xy(raw.dxf.insert)}
            attributes["content"] = str(cast("MText", raw).text)
    ...
    @staticmethod
    def _block_attributes(raw: Insert) -> dict[str, str]:
