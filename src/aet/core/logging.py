...
from aet.utils.ids import utc_now
...
class LogContext(TypedDict, total=False):
    """Context fields a caller may attach to an event (SDS-002 §11.2).

    Deliberately excludes the fields :class:`StructuredLogger` sets itself —
    severity, message, component, audit, timestamp. When context was plain
    ``**kwargs``, a call like ``logger.info(msg, audit="yes")`` bound its own
    context key to the routing parameter and misfiled a diagnostic record
    into the audit trail. Declaring the accepted keys makes that, and a
    misspelled key, a type error at the call site instead.
    """
...
class LogEvent:
    ...
    stage: str | None = None
    # Nullable like every other context field: an unattributed action is a
    # real possibility, and defaulting to "system" states the common case
    # without claiming the actor is always known.
...
class StructuredLogger:
    ...
    def _emit(
        ...
        *,
        ...
        """Build and route one structured event."""
        event = LogEvent(
    ...
    def trace(self, message: str, **context: Unpack[LogContext]) -> None:
        self._emit(TRACE, message, context)

    def debug(self, message: str, **context: Unpack[LogContext]) -> None:
        self._emit(logging.DEBUG, message, context)

    def info(self, message: str, **context: Unpack[LogContext]) -> None:
        self._emit(logging.INFO, message, context)

    def warn(self, message: str, **context: Unpack[LogContext]) -> None:
        self._emit(logging.WARNING, message, context)

    def error(self, message: str, **context: Unpack[LogContext]) -> None:
        self._emit(logging.ERROR, message, context)

    def fatal(self, message: str, **context: Unpack[LogContext]) -> None:
        self._emit(FATAL, message, context)

    def audit(self, message: str, **context: Unpack[LogContext]) -> None:
        """Record an immutable audit event (SDS-002 §11.5)."""
        self._emit(logging.INFO, message, context, audit=True)
